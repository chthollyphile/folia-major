...
// test/ui/automixTransitionSwitches.spec.ts
...
test('只拨圆环那个开关，圆环自己就演示一次', async ({ page }) => {
    await open(page);
...
test('混音中途关掉描边开关，描边立刻停', async ({ page }) => {
    await open(page);
...
for (const [name, which] of [['圆环', 'ring'], ['描边', 'card']] as const) {
    test(`混音跑到一半才拨上开关，${name}接着当前进度画完就走`, async ({ page }) => {
        await open(page);
...
test('圆环开着时拨卡片开关，圆环不会重来一遍', async ({ page }) => {
    await open(page);
...
test('描边正在画时拨圆环开关，描边不被打断', async ({ page }) => {
    await open(page);
