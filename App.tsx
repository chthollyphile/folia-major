...
export default function App() {
  ...
  }, [lyrics, currentLineIndex, audioPower, playerState, isDragging]);
                ...
                >
                    <motion.div layout className="w-full">
                        {showExpandedControls ? (
                            <div className="flex items-center gap-4 w-full">
                                ...
                                <div className="flex-1 flex flex-col justify-center gap-2 min-w-0 px-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] font-mono font-medium opacity-60 w-8 text-right" style={{color: 'var(--text-secondary)'}}>{formatTime(currentTime)}</span>
                                        ...
                                        <div className="relative h-1.5 flex-1 bg-white/10 rounded-full flex items-center group cursor-pointer">
                                            <div 
                                                ...
                                                style={{ width: `${duration ? (sliderValue / duration) * 100 : 0}%`, backgroundColor: 'var(--text-primary)' }} 
                                            />
                                            <input 
                                                type="range"
                                                ...
                                                onTouchEnd={(e) => {
                                                    if(audioRef.current) audioRef.current.currentTime = sliderValue;
                                                    setIsDragging(false);
                                                }}
                                                onClick={(e) => e.stopPropagation()}
                                        ...
                                        <span className="text-[10px] font-mono font-medium opacity-60 w-8" style={{color: 'var(--text-secondary)'}}>{formatTime(duration)}</span>
                                    </div>
                                </div>
                            ...
                            // Collapsed View
                            <div className="flex items-center gap-3 text-xs w-full justify-center h-8">
                                <span className="opacity-80 font-mono text-[10px]" style={{color: 'var(--text-secondary)'}}>{formatTime(currentTime)}</span>
                                
                                <div className="relative h-1 flex-1 bg-white/10 rounded-full flex items-center max-w-[150px] mx-2">
                                    <div 
                                        className="absolute top-0 left-0 h-full rounded-full" 
                                        style={{ width: `${duration ? (currentTime / duration) * 100 : 0}%`, backgroundColor: 'var(--text-primary)' }} 
                                    />
                                </div>

                                <span className="opacity-80 font-mono text-[10px]" style={{color: 'var(--text-secondary)'}}>{formatTime(duration)}</span>
                            </div>
                        )}
                    </motion.div>
                </motion.div>
